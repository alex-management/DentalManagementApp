...
const ViewPatientsModal: React.FC<ViewPatientsModalProps> = ({ isOpen, onClose, doctor }) => {
            ...
            >
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-gray-800 p-6 text-left align-middle shadow-xl transition-all">
                <Dialog.Title as="h3" className="text-lg font-medium leading-6 text-gray-900 dark:text-gray-100 flex justify-between items-center">
                  <span>Pacienți pentru {doctor.nume}</span>
                  <button onClick={onClose} className="p-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700">
                    <X size={20} />
                  </button>
                </Dialog.Title>
                <div className="mt-4">
                  {doctor.pacienti.length > 0 ? (
                    <ul className="max-h-60 overflow-y-auto space-y-2">
                      {doctor.pacienti.map(pacient => (
                        <li key={pacient.id} className="flex items-center p-2 rounded-md bg-gray-100 dark:bg-gray-700">
                          <User className="w-4 h-4 mr-3 text-gray-500" />
                          <span className="text-gray-800 dark:text-gray-200">{pacient.nume}</span>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-center text-gray-500 dark:text-gray-400 py-4">Acest doctor nu are pacienți înregistrați.</p>
                  )}
                </div>
